...
// returns the first n elements - signature included of the so seeded
// sequence.

// Signature will always contain 3 numbers; n will always be a
// non-negative number; if n == 0, then return an empty array
// (except in C return NULL) and be ready for anything else which
// is not clearly specified ;)

function tribonacci(signature, n) {
	if (n === 0) return (signature = []);
	if (n === 1) return (signature = [signature[0]]);
	if (n === 2) return (signature = [signature[0], signature[1]]);
	for (let i = 1; i <= n - 3; i++) {
		signature.push(signature[i - 1] + signature[i] + signature[i + 1]);
	}
	return signature;
}
